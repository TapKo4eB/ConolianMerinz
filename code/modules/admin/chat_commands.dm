...
/datum/tgs_chat_command/gameversion/Run(datum/tgs_chat_user/sender, params)
	...
	if (!GLOB.revdata)
		...
		if(GLOB.revdata.origindevcommit)
			msg += ", from origin commit: <[CONFIG_GET(string/githuburl)]/commit/[GLOB.revdata.origindevcommit]>"
	...
	return msg.Join("")
/**
 * I don't wanna bother myself with this for now
